...
async function main() {
  ...
  var response = await get_release_note(githubToken, repoName, productName);

  var release_note;
  var is_draft;
  var subject;
  var prettyName = productName != "" ? productName : to_title_case(repoName);
  ...
  // 3. send the email
  await send_email(emailPassword, emailUsername, emailRecipient, subject, release_note.body_html, hostName, emailSender, sendSecure)
}
...
try {
  ...
  sendSecure = core.getInput('send_secure');
